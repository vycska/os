...
int fs_checkdisk(void) {
   ...
   for(i = 0;
      i < DIRECTORY_ENTRIES && fs_filenamevalid(fs.dir[i].file_name) && fs.dir[i].max_sectors >= 0 && fs.dir[i].max_sectors < SECTORS && fs.dir[i].start_sector >= 0 && fs.dir[i].start_sector < SECTORS && fs.dir[i].header_size < MAX_HEADER_SIZE &&
