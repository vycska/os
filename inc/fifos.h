...
struct Fifo_HD44780 { //no checking if there is free space, just overwriting
	...
	int		i_get,i_put,
				length[FIFO_HD44780_ITEMS],line[FIFO_HD44780_ITEMS],pos[FIFO_HD44780_ITEMS],
				mtx_fifo_hd44780,
				smphr_count_items;
};
...
struct Fifo_Uart0_Output {
	...
	int		i_get,i_put,
				mtx_fifo_uart0_output,
				smphr_count_items,smphr_space_left,
				*smphr_finished[FIFO_UART0_OUTPUT_ITEMS];			
};
